...
def list_products(request):
    ...
    serializer = ProductSerializer(products, many=True)  # Convert to JSON format
    return Response(serializer.data)  # Send JSON response to frontend


# View to add a new product
# Receives product data from frontend and saves it to database
# Products are GLOBAL - they exist for all branches
# Branch and stock_quantity are OPTIONAL - if provided, creates initial stock for that branch
@api_view(['POST'])
def add_product(request):
    serializer = ProductSerializer(data=request.data)  # Convert JSON to Product object
